...
| `SOTTO_CORS_ORIGINS` | yes                                    | Expo web dev ports            | Comma-separated allowlist of browser `Origin` headers. Any `http://localhost:*`/`http://127.0.0.1:*` origin is also allowed **while `SOTTO_HOST` is a loopback address**. Once you bind `0.0.0.0` the server is reachable off-box, a localhost page is no longer necessarily your own, and this allowlist becomes the whole rule — so an exposed instance must list its real origin here.                                                                                                                                                                                                                               |
| `SOTTO_TRUST_PROXY`  | yes                                    | `0`                           | Set to `1` only when a trusted reverse proxy terminating TLS sits in front (Fly, Tailscale Serve, nginx). It makes the server honour `x-forwarded-for` and `x-forwarded-proto`. Two things depend on it: the per-IP session limiter (without it every request carries the proxy's address, so all callers share one bucket) and whether `POST /voice/session` returns a `ws://` or `wss://` URL (an https page refuses `ws://` as mixed content, so the tutor silently fails to connect). Leave `0` when nothing is in front: honouring those headers unproxied lets any client spoof its own IP past the rate limiter. |
...
| `SOTTO_STATIC_DIR`   | yes (this is what makes it one origin) | unset                         | Absolute path to a static web build (`apps/client/dist`, from `pnpm web:export`). When set, served at `/` (`index.html`, the landing page) with an SPA fallback to `app.html` (the Expo app shell) for any other unmatched GET path that doesn't look like a file, or to `index.html` if the export has no `app.html` (older exports) — mirrors `apps/client/scripts/serve-static.mjs`'s rule exactly. `/content/packs`, `/health`, `/voice/*`, `/import/*` are registered routes and always take precedence, regardless of this.                                                                                       |
