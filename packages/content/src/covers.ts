 ...
 * hand-authored art under `packages/content/covers/<bookId>.webp` (or the
 * PNG and legacy SVG fallbacks) gets that
...
/** Where the drawn covers live: one `<bookId>.webp`, `<bookId>.png`, or legacy
 * `<bookId>.svg` per book (art only, no text) plus `covers.json` describing
 * each. Data-driven — whatever is in this folder is authored, nothing is
 * listed in code. */
...
export function authoredCoverPath(
  ...
  for (const extension of ['.webp', '.png', '.svg']) {
    const file = path.join(coversDir, `${artBookId}${extension}`);
    if (existsSync(file)) return file;
  }
  return undefined;
}

/** The pack filename for authored art. Optimized WebP wins over PNG and SVG. */
export function authoredCoverFileName(
  artBookId: string,
  coversDir: string = COVERS_DIR,
): string | undefined {
  const source = authoredCoverPath(artBookId, coversDir);
  return source ? `cover${path.extname(source)}` : undefined;
}
 ...
 * every build (it is the source of truth, so it overwrites a same-format cover
 * generated by an earlier run); otherwise the deterministic SVG generator
 * fills in, and only when nothing is there yet.
 */
export function writeCover(
  dir: string,
  input: CoverInput,
  artBookId: string = input.bookId,
): { kind: 'authored' | 'generated' | 'kept'; file: string } {
  const authored = authoredCoverPath(artBookId);
  if (authored) {
    const file = authoredCoverFileName(artBookId)!;
    copyFileSync(authored, path.join(dir, file));
    return { kind: 'authored', file };
  }
  const file = 'cover.svg';
  const coverPath = path.join(dir, file);
  if (existsSync(coverPath)) return { kind: 'kept', file };
  writeFileSync(coverPath, generateCoverSvg(input), 'utf8');
  return { kind: 'generated', file };
}
