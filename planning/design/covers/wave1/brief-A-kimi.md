SUBTASK: Draw eight book-cover artworks as SVG for a graded-reader app (Sotto), direction "The Pressed Emblem".

INPUTS:
Direction anchor: Penguin Great Ideas / Pelican Books: one cut-paper emblem on a flat ground.
Art rule: ONE emblem per book that names the story in a single object (a windmill, a necklace, a cyclone funnel, a wolf's ear and a red hood, a bone flute, a stopped clock). Draw it as cut paper: flat fills, 2 to 3 tones plus the ground, no outlines. Compose it inside x 30..190, y 26..214, optically centred, generous margins. The ground is one solid tone filling the whole 220x330. Leave y 232..330 EMPTY (the app prints the title there); the only thing in that zone is the ground colour.
...
Text zone: the ground colour below y 232. The app prints the title over it, so it must be plain.
