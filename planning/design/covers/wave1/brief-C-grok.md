SUBTASK: Draw eight book-cover artworks as SVG for a graded-reader app (Sotto), direction "The Endpaper".

INPUTS:
Direction anchor: Coralie Bickford-Smith Clothbound Classics: one motif tile repeated as cloth.
Art rule: One motif tile per book, drawn from the story (feathers for Poe, tiny diamonds for the necklace, windmill sails, spiral funnels, red hoods, bone flutes, brush-stroke idiom strokes for the chengyu book, clock faces for Larra), repeated as a pattern with <pattern> or explicit repetition to fill the whole 220x330. Two to three tones plus the ground; the tile is 28 to 44 units so it still reads at 104px wide. Then draw ONE label rectangle, fill #F4ECDF, at x 24, y 210, width 172, height 90, with a 2-unit inset rule in the ground tone; leave the label empty (the app prints the title on it).
...
Text zone: the label rectangle (always ink #221E1B text on #F4ECDF). The app prints the title over it, so it must be plain.
