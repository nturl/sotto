...
- 2026-09-08 17:53 EDT: `fly deploy --app sotto-cloud` from `c6ebabb`, image `deployment-01M21FJ58Z8FFR84BHGP5RTRWG`, machine `84391ea2d15038` updated in place, health passing. Live: `/health` 200, `/content/packs` JSON, `/auth/config` `{"magicLink":true}`, `GET /billing/plans` with `Origin: https://readsotto.app` answers `access-control-allow-origin: https://readsotto.app`, and the signed-out root lands on `/account?intent=start` showing "Create your free account" at 375 and 1440.
- **Found on the way:** before this deploy the paid client live at app.readsotto.app (the other session's 13:03 build, Fly v26) had been exported without `EXPO_PUBLIC_CLOUD_URL`, so it ran as the free build and "Try the tutor free for 3 days" landed on "This feature isn't available in this build." The Dockerfile's `ARG PUBLIC_ORIGIN` / `ENV EXPO_PUBLIC_CLOUD_URL` recipe is correct and a local export with the variable set inlines `new HttpCloudAdapter("https://app.readsotto.app", …)`; how that build lost it is not known (their deploy command was not seen). Worth a guard: a post-deploy check that the served bundle contains the adapter, or a build-time assertion in `build-web.mjs` when `EXPO_PUBLIC_CLOUD_URL` is expected.
- **Found on the way, carried:** with a saved OpenAI key on readsotto.app, Noel's Discuss turn showed "Could not reach OpenAI…" twice. Probed from the deployed page: the browser reaches `api.openai.com` (a readable 401 on `/v1/models`, with and without the service worker), and nothing shipped today touched the BYOK request path. OpenAI's error responses on the inference endpoints carry no CORS header, so a 401, 429 or 404 from `gpt-transcribe`, `gpt-5.6-terra` or `gpt-4o-mini-tts` all surface as that one message, and the Settings key test (`/v1/models`) cannot see quota or model access. The app should surface a readable cause; until it does, the browser's Network tab is the diagnostic.
- 2026-09-08 evening, after Noel's note: the landing's two "See it working" screenshots were re-shot from the live app and re-embedded (`embed-shots.mjs`); the Library shot had been captured before the parchment covers merged and still showed the previous cover art.
